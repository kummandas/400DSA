...
int findLevel(TreeNode<int> *root, int target, int level) {
    if(root == NULL)
        return -1;
    if(root->val == target)
        return level;
...
{
    TreeNode<int> *ancestor = lca(root,node1,node2);
    int d1 = findLevel(ancestor,node1,0);
    int d2 = findLevel(ancestor,node2,0);
    int ans = d1+d2;
    if(ans < 0)
        return -1;
    return ans;
